...
ma <- read.csv(file.path(PROJECTDIR,'DATA','COMP_SRTM_data_ma_a.csv'),sep='\t',header=T,dec='.')
ma2 <- read.csv(file.path(PROJECTDIR,'DATA','COMP_SRTM_data_ma_b.csv'),sep='\t',header=T,dec='.')
...
p<-p+ylab('Difference compared to SRTM15+ v2.2 Depth [m]')+xlab('IBCSO v2 Depth [m]')
...
png(file.path(PROJECTDIR,"fig 6b.png"),3200,3200,res=600)
print(p)
graphics.off()
